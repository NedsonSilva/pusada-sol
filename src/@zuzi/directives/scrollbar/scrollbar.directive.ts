...
import { ScrollbarGeometry, ScrollbarPosition } from '@zuzi/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[zuziScrollbar]',
    exportAs: 'zuziScrollbar',
})
export class ZuziScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    static ngAcceptInputType_zuziScrollbar: BooleanInput;

    @Input() zuziScrollbar: boolean = true;
    @Input() zuziScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('zuziScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.zuziScrollbar = coerceBooleanProperty(
                changes.zuziScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.zuziScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('zuziScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.zuziScrollbarOptions.currentValue
            );
