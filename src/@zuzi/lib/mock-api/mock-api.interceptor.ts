...
import { ZuziMockApiService } from '@zuzi/lib/mock-api/mock-api.service';
import { delay, Observable, of, switchMap, throwError } from 'rxjs';

@Injectable({
    providedIn: 'root',
})
export class ZuziMockApiInterceptor implements HttpInterceptor {

    constructor(
        @Inject(ZUZI_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _zuziMockApiService: ZuziMockApiService
    ) {}
    ...
    intercept(
        ...
        const { handler, urlParams } = this._zuziMockApiService.findHandler(
